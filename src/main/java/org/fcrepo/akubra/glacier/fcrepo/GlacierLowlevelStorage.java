...
import org.fcrepo.server.storage.lowlevel.akubra.PackageProxy;
 ...
 * @author Chris Wilper
 */
public class GlacierLowlevelStorage
        implements ILowlevelStorage, IListable, ISizable {

    private static final Logger logger =
            LoggerFactory.getLogger(GlacierLowlevelStorage.class);

    private static Map<String, String> GLACIER_HINT = getGlacierHint();
    
    private final BlobStore objectStore;

    private final GlacierBlobStore datastreamStore;
     ...
     */
    public GlacierLowlevelStorage(BlobStore objectStore,
                                 GlacierBlobStore datastreamStore,
    ...
    private static Iterator<String> list(BlobStore store) {
        ...
        try {
            ...
            successful = true;
            return PackageProxy.getKeyIterator(connection, blobIds);
        } finally {
            if (!successful) {
                closeConnection(connection);
            }
    ...
    private static InputStream retrieve(BlobStore store,
        ...
        try {
            URI blobId = getBlobId(key);
            Map<String, String> hints = new HashMap<String, String>(1);
            hints.put(GlacierBlobStore.DS_ID_KEY, blobId.toString());
            connection = getConnection(store, hints);
            Blob blob = getBlob(connection, blobId, hints);
            content = openInputStream(blob);
            successful = true;
            return PackageProxy.wrapStream(connection, content);
    ...
    private static URI getBlobId(String token) {
        try {
        	return new URI(getBlobIdString(token));
    ...
    private static String getBlobIdString(String token) {
        try {
            int i = token.indexOf('+');
            if (i == -1) {
                return new PID(token).toURI();
            } else {
    ...
    private static Map<String, String> getGlacierHint() {
    	HashMap<String, String> hint = new HashMap<String, String>(1);
    	hint.put(GlacierBlobStore.STORAGE_HINT_KEY, GlacierBlobStore.STORAGE_HINT_VALUE_GLACIER);
